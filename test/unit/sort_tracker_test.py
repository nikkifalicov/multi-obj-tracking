...
class SORTTrackerTests(unittest.TestCase):
    """
    Tests to verify the SORTTracker is working.
    """
    ...
    def test_sort_tracker_basic_e2e(self, return_mode, update_method):  # pylint: disable=too-many-locals
        """
        Basic e2e test of the SORT tracker with different input types on the person example with ground truth data
        """
        ...
        tracker = SORTTracker(image_width=image_width, image_height=image_height)
    ...
    def test_sort_tracker_on_volleyball_example_groundtruth(
        self, should_save_video: bool = False
    ):  # pylint: disable=too-many-locals
        """
        Test the SORT tracker on the groundtruth data from the volleyball example (multi-object and multi-class)
        """
        dataset, image_width, image_height = create_mock_sdk_provider(
        ...
        if should_save_video:
            ...
            video_path = "test/assets/volleyball_example/sort_tracker_on_groundtruth_demo.mp4"
        ...
        assert_metrics(
            ...
            {
                "mota": 0.90,
                # we know that SORT will switch tracks frequently on this example due to occulusion and change of
                # direction
                "num_switches": (0, 16),
    ...
    def test_sort_tracker_on_gl_inferences_person_example(
        self, should_save_video: bool = False
    ):  # pylint: disable=too-many-locals
        """
        Test the SORT tracker on the GL inferences on the person example
        ...
        for frame_idx, (image, data, _) in enumerate(dataset):
            ...
            for track in tracks_by_frame[frame_idx]:
                assert track.confidence is None, (
                    f"Confidence should be None for track {track.id} at frame {frame_idx} as the SORT tracker does "
        ...
        if should_save_video:
            # save the annotated images as a video
            video_path = "test/assets/person_example/sort_tracker_on_gl_inferences_demo.mp4"
            fourcc = cv2.VideoWriter_fourcc(*"mp4v")  # type: ignore
            writer = cv2.VideoWriter(video_path, fourcc, 30, (image_width, image_height))
            try:
                for annotated_image in annotated_images:
    ...
    def test_sort_tracker_on_gl_inferences_volleyball_example(
        self, should_save_video: bool = False
    ):  # pylint: disable=too-many-locals
        """
        Test the SORT tracker on the GL inferences on the volleyball example
        """
        dataset, image_width, image_height = create_mock_sdk_provider(
        ...
        if should_save_video:
            # save the annotated images as a video
            video_path = "test/assets/volleyball_example/sort_tracker_on_gl_inferences_demo.mp4"
            fourcc = cv2.VideoWriter_fourcc(*"mp4v")  # type: ignore
            writer = cv2.VideoWriter(video_path, fourcc, 30, (image_width, image_height))
            try:
                for annotated_image in annotated_images:
    ...
    def test_reset(self):
        """
        Verifies that the SORT tracker's reset method correctly resets all internal state,
        including calling reset on the wrapped tracker.
        """

        tracker = SORTTracker(image_width=100, image_height=100)
    ...
    def test_predicted_tracks_use_cached_class_names_and_frames_since_last_update_increments(self):
        ...
        assert track.confidence is None, "Confidence should be None for all tracks from SORT"
    ...
    def test_max_frames_since_last_update_filtering(self):
        ...
        """
        tracker = SORTTracker(image_width=100, image_height=100, lost_track_buffer=5, minimum_consecutive_frames=1)
